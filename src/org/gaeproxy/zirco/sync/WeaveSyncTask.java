...
public class WeaveSyncTask extends AsyncTask<WeaveAccountInfo, Integer, Throwable> {
  ...
  protected Throwable doInBackground(WeaveAccountInfo... arg0) {
    ...
    try {
      ...
      UserWeave userWeave =
          getWeaveFactory().createUserWeave(accountInfo.getServer(), accountInfo.getUsername(),
              accountInfo.getPassword());

      long lastModifiedDate = getLastModified(userWeave).getTime();
      long lastSyncDate = PreferenceManager.getDefaultSharedPreferences(mContext)
          .getLong(Constants.PREFERENCE_WEAVE_LAST_SYNC_DATE, -1);
  ...
  private void doSync(WeaveAccountInfo accountInfo, UserWeave userWeave,
    ...
    for (WeaveBasicObject wbo : wboList) {
      ...
      if (decryptedPayload.has(WEAVE_HEADER_TYPE) && ((decryptedPayload.getString(WEAVE_HEADER_TYPE)
          .equals(WEAVE_VALUE_BOOKMARK)) || (decryptedPayload.getString(WEAVE_HEADER_TYPE)
        ...
        if (decryptedPayload.has(WEAVE_VALUE_TITLE)) {
          ...
          String weaveId =
              decryptedPayload.has(WEAVE_VALUE_ID) ? decryptedPayload.getString(WEAVE_VALUE_ID)
                  : null;
          String parentId =
              decryptedPayload.has(WEAVE_VALUE_PARENT_ID) ? decryptedPayload.getString(
                  WEAVE_VALUE_PARENT_ID) : null;
  ...
  private void doSyncByDelta(WeaveAccountInfo accountInfo, UserWeave userWeave,
      List<WeaveBasicObject> wboList)
      throws WeaveException, JSONException, IOException, GeneralSecurityException {
      ...
      if (decryptedPayload.has(WEAVE_HEADER_TYPE)) {
        ...
        if (decryptedPayload.getString(WEAVE_HEADER_TYPE).equals(WEAVE_VALUE_ITEM)
          ...
          if ((weaveId != null) && (weaveId.length() > 0)) {
            ...
            long id = BookmarksProviderWrapper.getWeaveBookmarkIdByWeaveId(mContentResolver,
  ...
  private QueryResult<List<WeaveBasicObject>> getCollection(UserWeave weave, String name,
      QueryParams params) throws WeaveException {
